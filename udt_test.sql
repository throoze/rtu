...
hito_t.crearHito('La estatua del libertador','Estatua Simon Bolivar',tabla_tipoHito_t('Academico','Cultural'),'Abierto',lista_costos_t(costo_t('Ninos',200),costo_t('Adultos',300)),'Todo publico',20,'Formal');
hito_t.crearHito('Biblioteca de la USB','Biblioteca central',tabla_tipoHito_t('Academico'),'Abierto',lista_costos_t(costo_t('General',0)),'Todo publico',16,'Casual');
INSERT INTO Turista VALUES('Si','Perez','contrasena',SYSDATE, 'Hombre', 'correo@correo.com', tabla_tipoHito_t('Academico'), 'Pedro','pedrito');
SELECT REF(t) into tu_r FROM Turista t WHERE username='pedrito';
SELECT REF(h) bulk collect into th FROM Hito h;
INSERT INTO Ruta VALUES (SYSDATE, 'Conoce la Simon', tabla_tipoHito_t('Academico','Cultural'),(SELECT REF(t) FROM Turista t WHERE username='pedrito'),tabla_hito_t((SELECT REF(h) FROM Hito h)));
